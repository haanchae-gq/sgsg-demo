...
7. **자동 재배정 로직 실행**:
   - 배정 로직 입력 조건:
     ...
     - 서비스 카테고리
     - `channel_id` (채널 정보)
     - `channel_type` (INTERNAL/PARTNER - 배정 정책 분기 조건으로 활용 가능)
     - 활성 전문가
     - 기존 거절한 전문가 제외
     - 멤버십 우선 정책 적용
   - **채널 연동 참고**: 채널 정보(`channel_id`, `channel_type`)는 배정 엔진으로 전달되며, v1 기준으로는 채널 무관 동일 배정 정책이 적용됩니다. 향후 채널 타입 기반 조건 분기 또는 특정 채널 예외 정책 추가 가능 (채널 관리 문서 시나리오 12 참조).
   - 다음 전문가 후보 선정:
     - 멤버십 전문가가 존재하는 경우:
...
4. **환불 가능 여부 판단 및 정책 적용**:
   ...
   - 환불 정책 판단:
     ...
     - 예약금 환불
     - 단순 금액 조정 환불
     - 채널별 수수료 정책 반영 (채널별 수수료 설정에 따라 환불 금액 계산 및 정산 반영)
