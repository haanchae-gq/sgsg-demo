...
### 채널 관리 연동
주문 관리 모듈은 채널 관리 모듈과 긴밀히 연동되어 있습니다:

- **채널 기반 주문 구분**: 모든 주문은 특정 채널(`channel_id`)에 속하며, 채널 타입(`channel_type`)에 따라 INTERNAL(자사) 또는 PARTNER(제휴)로 구분됩니다.
- **주문 생성 시 채널 선택**: 신규 주문 생성 시 반드시 채널을 선택해야 합니다. 선택 가능한 채널은 `channel_status = ACTIVE` 상태인 채널만 드롭다운에 표시됩니다.
- **엑셀 업로드 연동**: 주문 엑셀 업로드 시 `channel_code` 컬럼을 통해 채널 매핑이 수행됩니다. INACTIVE 채널의 경우 업로드가 실패 처리됩니다.
- **주문 상세 화면**: 주문 상세 화면에는 채널명(`channel_name_snapshot`), 채널 타입, 채널 주문번호(`channel_order_no`)가 표시됩니다.
- **필터링**: 주문 목록에서 채널명, 채널 타입(INTERNAL/PARTNER) 기준 필터링이 가능합니다.
- **데이터 무결성**: 채널의 `channel_code`, `channel_name`, `channel_type`은 생성 후 수정 불가능하여 주문 데이터의 참조 무결성이 보장됩니다.
- **상태 영향**: 채널이 INACTIVE로 전환되면 신규 주문 생성이 차단되지만, 기존 주문은 정상적으로 진행됩니다.
